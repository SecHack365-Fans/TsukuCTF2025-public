---
title: PQC2
description: 
author: hiikunZ
genre: crypto
solver: 0 
point: 0
---

# PQC2

## 問題文
今度は秘密鍵の最初の 294 バイトをもらいます!
## 難易度
Hard
## 解法

[仕様書](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf) を読んで実装を頑張って、LLL などで復元してください。

各情報が NTT 変換される前/後のどちらの情報なのかに注意が必要です。
また、$$ \mathrm{NTT}^{-1}(\boldsymbol{\hat{a}}) + \mathrm{NTT}^{-1}(\boldsymbol{\hat{b}}) = \mathrm{NTT}^{-1}(\boldsymbol{\hat{a}} + \boldsymbol{\hat{b}}) $$ であることを考慮して格子の次数を落とさないと、時間的に厳しいと思われます。(これは、inv_NTT の実装を見るとほぼ明らかです。)

以下に LLL で解く方法の一例を示します。

$a_{i,j}$ は $v =  (x^i,0,0)$ として $ Av $ の $j$ 次の項の係数です。

$(A s' - t)_{i}$ は $s'$ を $\hat{s}$ の不明な部分の係数を 0 にして inv_NTT にかけた結果として $A s' - t$ の $i$ 次の項の係数です。

$$\begin{pmatrix}
C \cdot a_{0,0} & C \cdot a_{0,1} & \cdots & C \cdot a_{0,Y-1} & 1 & 0 & \cdots & 0 & 0\\
C \cdot a_{1,0} & C \cdot a_{1,1} & \cdots & C \cdot a_{1,Y-1} & 0 & 1 & \cdots & 0 & 0\\
\vdots           & \vdots           &        & \vdots             &   &   & \ddots &   & \vdots\\
C \cdot a_{X-1,0} & C \cdot a_{X-1,1} & \cdots & C \cdot a_{X-1,Y-1} & 0 & 0 & \cdots & 1 & 0\\
C \cdot q       & 0                & \cdots & 0                   & 0 & 0 & \cdots & 0 & 0\\
0                 & C \cdot q      & \cdots & 0                   & 0 & 0 & \cdots & 0 & 0\\
\vdots            &                  & \ddots &                     &   &   &        &   & \vdots\\
0                 & 0                & \cdots & C \cdot q         & 0 & 0 & \cdots & 0 & 0\\
C \cdot (A s' - t)_0 & C \cdot (A s' - t)_1 & \cdots & C \cdot (A s' - t)_{Y-1} & 0 & 0 & \cdots & 0 & C \cdot 10000
\end{pmatrix}.
$$

これを格子縮約すると、
$(C \cdot e_{0,0},C\cdot e_{0,1}, \cdots , C \cdot e_{0,Y-1}, \hat{s}_{0,0},\hat{s}_{0,1}, \hat{s}_{0,X-1},C \cdot 10000)$ という行が結果に現れることが期待できます。($e_{0,i}$ は $e_0$ の $j$ 次の項の係数、$\hat{s}_{0,i}$ は $\hat{s}_0$ の $i$ 次の項の係数)

sagemath での実装の一例を示します。なお [kyber-py](https://github.com/GiacomoPope/kyber-py/) を使っています。

```python
import base64
from Crypto.Hash import SHAKE128
from kyber_py.ml_kem import ML_KEM_768
from Crypto.Cipher import AES
from Crypto.Util.Padding import unpad

key = "A" * 262 + """duCDHV8l8e03uLKAf8MJgR2IbHujeYwiPkaHjkGz83AE0FIWSmgIxgVr6F
"""

key = base64.b64decode(key)

ciphertext = bytes.fromhex("")
encrypted_flag = bytes.fromhex("bed3b7d98a1058fe7059c15bffac13205a39bc22263ef9110b5bde66f10c847fbe2eae728a1a427d99bbee0b48c9fd76")


n = 256
q = 3329
k = 3

def sampleNTT(B):
    ctx = SHAKE128.new()
    ctx.update(B)
    j = 0
    a = []
    while j < 256:
        C = ctx.read(3)
        d1 = C[0] + 256 * (C[1] % 16)
        d2 = (C[1] // 16) + 16 * C[2]
        if d1 < q:
            a.append(d1)
            j += 1
        if d2 < q and j < 256:
            a.append(d2)
            j += 1
    return a

def BitsToBytes(b):
    B = []
    for i in range(len(b) // 8):
        x = 0
        for j in range(8):
            x += b[i * 8 + j] << j
        B.append(x)
    return bytes(B)

def BytesToBits(B):
    b = []
    for i in range(len(B)):
        for j in range(8):
            b.append((B[i] >> j) & 1)
    return b

def ByteEncode(F):
    b = []
    for i in range(256):
        x = F[i]
        for j in range(12):
            b.append((x >> j) & 1)
    B = BitsToBytes(b)
    return B

def ByteDecode(B):
    b = BytesToBits(B)
    F = []
    for i in range(256):
        x = 0
        for j in range(12):
            x += b[i * 12 + j] << j
        assert x < q
        F.append(x)
    return F

def bitrev(i):
    bin_i = bin(i & (2**7 - 1))[2:].zfill(7)
    return int(bin_i[::-1], 2)

def inv_NTT(f):
    i = 127
    l = 2
    while l <= 128:
        s = 0
        while s < 256:
            zeta = pow(17, bitrev(i), q)
            i = i - 1
            for j in range(s,s + l):
                t = f[j]
                f[j] = t + f[j + l]
                f[j + l] = zeta * (f[j + l] - t)
            s += 2 * l
        l *= 2
    for i in range(256):
        f[i] = (f[i] * 3303) % q
    return f

s_enc = key[98:1250]
t_enc = key[1250:2402]
rho = key[2402:2434]

A = [[0] * k for _ in range(k)]
for i in range(k):
    for j in range(k):
        A[i][j] = sampleNTT(rho + bytes([j, i]))


s = [ByteDecode(s_enc[0:384]), ByteDecode(s_enc[384:768]), ByteDecode(s_enc[768:1152])]

X = 66

for i in range(X):
    s[0][i] = 0

t = [ByteDecode(t_enc[0:384]), ByteDecode(t_enc[384:768]), ByteDecode(t_enc[768:1152])]

PR.<x> = PolynomialRing(GF(q))
R.<z> = PR.quotient_ring(x^n + 1)

for i in range(k):
    for j in range(k):
        A[i][j] = R(inv_NTT(A[i][j]))

for i in range(k):
    s[i] = R(inv_NTT(s[i]))
    t[i] = R(inv_NTT(t[i]))

A = matrix(R, A)
s = vector(R, s)
t = vector(R, t)

Y = 132
C = 30000

M = matrix(ZZ,Y + X+1,Y + X+1)

for i in range(X):
    a = [0] * 256
    a[i] = 1
    a = R(inv_NTT(a))
    a = vector(R, [a,0,0])
    M[i,:Y] = C * vector(ZZ,(A * a)[0].list()[:Y])

M[:X,Y:Y+X] = identity_matrix(ZZ,X,X)
M[X:Y+X,:Y] = C * q * identity_matrix(ZZ,Y,Y)
M[Y+X,:Y] = C * vector(ZZ, (A * s - t)[0].list()[:Y])
M[Y+X,Y+X] = C * 10000

M = M.LLL()


s0 = ByteDecode(s_enc[0:384])

for i in range(X):
    s0[i] = int(M[-1,Y + i]) % q

dk = ByteEncode(s0) + key[98 + 384:]

shared_secret = ML_KEM_768.decaps(dk, ciphertext)

flag = unpad(AES.new(shared_secret, AES.MODE_ECB).decrypt(encrypted_flag), 16)
print(flag.decode())
```
